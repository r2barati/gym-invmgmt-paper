"""
gnn_il_agent.py — GNN-IL (Behavioral Cloning → PPO Fine-tune) Evaluation Agent

Architecture identical to GNN V3 (same GNNFeaturesExtractorV3), but with
weights initialized via Oracle BC then PPO-finetuned.

Matches training pipeline:
  train_gnn_il.py
  Model: ppo_gnn_il_best.zip + vec_normalize_gnn_il.pkl
"""
...
class GNNILAgent:
    def __init__(self, env, model_path: str = 'data/models/ppo_gnn_il_best.zip',
                 stats_path: str = 'data/models/vec_normalize_gnn_il.pkl',
        ...
        # Same feature wrapper as GNN V3: enhanced=True, grouped=True
